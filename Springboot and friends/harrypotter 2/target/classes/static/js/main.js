...
var findAll = function () {
    ...
    $.ajax({
        ...
        success: function(data) {
            renderList(data);
            populateDropdown(data);  // Call function to populate dropdown
        },
...
var populateDropdown = function(data) {
    let dropdownMenu = $("#bookDropdownMenu");
    dropdownMenu.empty(); // Clear previous items

    $.each(data, function(_, book) {
        let item = `<a class="dropdown-item bookOption" data-id="${book.id}" href="#">${book.title}</a>`;
...
$(document).ready(function(){
	...
	$(document).on("click", ".bookOption", function() {
	    ...
	    findById(bookId); // Fetch and display book details in modal
	});
